...
main args:
  with-mock-sync: | sync |
    run-client-test args --use-mock: test it --sync=sync

test client/LspClient --sync/MockSync:
  ...
  mock-compiler.set-analysis-result --sync=sync error-answer
  client.send-did-open --uri=uri --text="""
    Completely ignored content.
  """
  sync.wait-for-waiting 1
